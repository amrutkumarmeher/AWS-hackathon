function getApiUrl(endpoint) {
  if (
    window.MEALSYNC_CONFIG &&
    typeof window.MEALSYNC_CONFIG.apiUrl === "function"
  ) {
...
window.fetch = function (resource, options) {
  ...
  if (
    ...
    (resource.startsWith("/api") || resource === "/api")
  ) {
    ...
    resource.url.startsWith("/api")
  ) {
    resource = new Request(
      getApiUrl(resource.url.replace(window.location.origin, "")),
      resource,
    );
  }
...
function evaluateNotificationRules(student, counter) {
  ...
  if (pos === 3 && state.lastNotifiedPosition > 3) {
    ...
    toast(
      `🔔 Only 2 people ahead at ${counter.name}! Head to the line.`,
      "success",
    );
  }
  ...
  if (pos === 1 && state.lastNotifiedPosition > 1) {
    ...
    toast(
      `🍽️ You are #1 at ${counter.name}! Please collect your plate.`,
      "success",
    );
  }
...
async function setupNotificationPermission() {
  if (!("Notification" in window)) {
    toast(
      "Browser does not support notifications. In-app alerts will be used.",
      "warning",
    );
  ...
  if (Notification.permission === "granted") {
    ...
    toast(
      state.notificationsEnabled
        ? "Notifications active! You will be alerted when 2 people are ahead."
    ...
    if (state.notificationsEnabled) {
      triggerBrowserNotification(
        ...
        "We will notify you when 2 people remain ahead of you in line.",
      );
    }
    return;
  }
  ...
  if (Notification.permission !== "denied") {
    ...
    if (permission === "granted") {
      state.notificationsEnabled = true;
      toast(
        "Notifications enabled! You will be alerted when 2 people are ahead.",
        "success",
      );
      triggerBrowserNotification(
        "Notifications Active 🔔",
        "Ready! You will get an alert when 2 people are ahead of you.",
      );
    } else {
      toast(
        "Permission declined. In-app alerts will still be shown.",
        "warning",
      );
    ...
    toast(
      "Notifications are blocked in browser settings. Please allow notifications for this page.",
      "warning",
    );
    state.notificationsEnabled = false;
  }
...
function updateNotifyUI() {
  ...
  if (state.notificationsEnabled) {
    ...
    if (sub)
      sub.textContent =
        "Active: Chrome will notify you when 2 students are ahead of you in line!";
    ...
    if (sub)
      sub.textContent =
        "Get a native alert when 2 students are ahead of you in line.";
...
async function fetchCounters() {
  try {
    ...
    if (data.success) {
      ...
      if (data.upcomingSlot !== undefined)
        state.upcomingSlot = data.upcomingSlot;
...
async function checkStudentStatus() {
  ...
  try {
    const res = await fetch(
      `/api/student-status?studentId=${encodeURIComponent(state.user.id)}`,
    );
    const data = await res.json();
...
function renderSchedulesTable() {
  ...
  if (!state.schedules || !state.schedules.length) {
    tbody.innerHTML =
      '<tr><td colspan="5" style="text-align:center;color:#64748b;padding:1.5rem;">No scheduled slots defined.</td></tr>';
    return;
  }
...
window.adminToggleSchedule = async function (id) {
  try {
    const res = await fetch(`/api/admin/schedules/${id}/toggle`, {
      method: "POST",
    });
    const d = await res.json();
    if (d.success) {
...
function renderHomeMenu() {
  ...
  const countersWithMenu = state.counters.filter((c) => {
    ...
    const validItems = c.menu.filter(
...
function renderHomeStats() {
  ...
  if ($("homeStatActiveCounters"))
    $("homeStatActiveCounters").textContent = count;
  if ($("homeStatTotalQueue")) $("homeStatTotalQueue").textContent = qTotal;
  if ($("homeStatAvgPace"))
    $("homeStatAvgPace").textContent =
      (count > 0 ? Math.round(paceSum / count) : 40) + "s";
  if ($("homeStatMealsServed"))
...
function renderTicket() {
  ...
  $("ticketFoodBadge").className =
    `badge-food-pill ${isVeg ? "veg" : "non-veg"}`;
  $("ticketFoodBadge").textContent = isVeg ? "Pure Veg" : "Non-Veg";

  $("ticketPosition").textContent = `#${student.position}`;
  $("ticketPositionNote").textContent =
    student.position === 1
      ? "You are next in line! Collect your meal."
      : student.position === 3
        ? "🔔 Only 2 people ahead of you!"
        : `${student.position - 1} student(s) ahead of you`;

  const estSec = Math.max(
    0,
    (student.position - 1) * (counter.avgServeSeconds || 40),
  );
  $("ticketCountdown").textContent = fmtWait(estSec);
  $("ticketPaceNote").textContent =
    `Speed: ~${counter.avgServeSeconds || 40}s per student`;

  const elapsed = Math.max(
    0,
    Math.floor((Date.now() - student.joinedAt) / 1000),
  );
  $("ticketElapsed").textContent = fmtTime(elapsed);

  const menuItems = (counter.menu || []).filter(
    (m) => m && String(m).trim().length > 0,
  );
  $("ticketMenuList").textContent = menuItems.length
    ? menuItems.join(", ")
    : "Daily Mess Meal";
}
...
function renderStudentCounters() {
  ...
  const list = state.counters.filter(
    (c) => state.filter === "all" || c.foodType === state.filter,
  );
  if (!list.length) {
    cont.innerHTML =
      '<div class="empty-state-card"><p>No mess counters available.</p></div>';
    return;
  }

  const userCounterId =
    state.queueStatus.inQueue && state.queueStatus.counter
      ? state.queueStatus.counter.id
      : null;
  ...
  list.forEach((c) => {
    ...
    const menuItems = (c.menu || []).filter(
      (m) => m && String(m).trim().length > 0,
    );
    const menuSnippet = menuItems.length
...
function renderStaffCounters() {
  ...
  state.counters.forEach((c) => {
    ...
    `,
          ...
          .join("")
      : '<tr><td colspan="5" style="text-align:center;color:#64748b;padding:1rem;">No students in queue.</td></tr>';
...
setInterval(() => {
  if (state.queueStatus.inQueue && state.queueStatus.student) {
    const elapsed = Math.max(
      0,
      Math.floor((Date.now() - state.queueStatus.student.joinedAt) / 1000),
    );
...
window.serveNext = async function (id) {
  try {
    const res = await fetch(`/api/counters/${id}/serve-next`, {
      method: "POST",
    });
    const d = await res.json();
    if (d.success) {
      toast(
        `Served ${d.servedStudent.studentName} (${d.durationSeconds}s).`,
        "success",
      );
      await fetchCounters();
      await checkStudentStatus();
...
function renderAdminStats(stats) {
  if (!stats) return;
  if ($("adminPillMaxQ"))
    $("adminPillMaxQ").textContent = stats.defaultMaxQueueSize || 20;
  if ($("adminPillTotalServes"))
    $("adminPillTotalServes").textContent = stats.totalServesAllTime || 0;
  if ($("inputGlobalMaxQueue"))
    $("inputGlobalMaxQueue").value = stats.defaultMaxQueueSize || 20;
  if ($("inputAdminAnnouncement"))
    $("inputAdminAnnouncement").value = stats.announcement || "";
}
...
function renderAdminModeration(countersList) {
  ...
  if (!countersList || !countersList.length) {
    cont.innerHTML =
      '<div class="empty-state-card"><p>No counters available.</p></div>';
    return;
  }
  ...
  countersList.forEach((c) => {
    ...
    `,
          ...
          .join("")
      : '<tr><td colspan="5" style="text-align:center;color:#64748b;padding:0.75rem;">No students in queue.</td></tr>';
...
window.adminReopenCounter = async function (id) {
  try {
    const res = await fetch(`/api/admin/counters/${id}/reopen`, {
      method: "POST",
    });
    const d = await res.json();
    if (d.success) {
...
window.adminClearQueue = async function (counterId, counterName) {
  ...
  try {
    const res = await fetch(`/api/admin/counters/${counterId}/clear`, {
      method: "POST",
    });
    const d = await res.json();
    if (d.success) {
...
async function fetchAuditLogs() {
  const search = $("auditSearchInput")
    ...
    : "";
  const counterId = $("auditCounterSelect")
    ? $("auditCounterSelect").value
    : "";
...
function renderAuditLogsTable(serves) {
  ...
  if (!serves || !serves.length) {
    tbody.innerHTML =
      '<tr><td colspan="7" style="text-align:center;color:#64748b;padding:2rem;">No meal serve records found.</td></tr>';
    return;
  }
...
function setAdminTab(tabName) {
  ...
  [
    "adminTabContentModeration",
    "adminTabContentScheduling",
    "adminTabContentSettings",
    "adminTabContentAuditLogs",
  ].forEach((id) => {
    const el = $(id);
    if (el) el.style.display = "none";
  });
...
$("btnNavStudentLogin").addEventListener("click", () =>
  setView("studentLogin"),
);
...
$("btnTestNotify").addEventListener("click", () => {
  triggerBrowserNotification(
    "Test Notification 🔔",
    "This is a test alert from MealSync. Chrome notifications are working!",
  );
});
...
$("tabBtnModeration").addEventListener("click", () =>
  setAdminTab("moderation"),
);
$("tabBtnScheduling").addEventListener("click", () =>
  setAdminTab("scheduling"),
);
...
function isMealSyncHealth(data) {
  return !!(
    data &&
    (data.status === "healthy" || data.service === "MealSync API")
  );
}
    ...
    es.addEventListener("student_served", (e) => {
      ...
      try {
        ...
        if (
          ...
          payload.servedStudent &&
          payload.servedStudent.studentId === state.user.id
        ) {
          triggerBrowserNotification(
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  if (livePill) {
    ...
    livePill.addEventListener("click", () => {
      const current =
        (window.MEALSYNC_CONFIG && window.MEALSYNC_CONFIG.API_BASE_URL) ||
        window.location.origin;
      const input = prompt(
        `MealSync Backend API URL:\nCurrent target: ${current}\n\nEnter new Backend URL (or leave empty to reset to default):`,
        current,
      );
