(function (root) {
  ...
  let storedApiUrl =
    typeof localStorage !== "undefined"
      ? localStorage.getItem("MEALSYNC_API_BASE_URL")
      : null;
  ...
  const windowApiUrl =
    typeof window !== "undefined" ? window.MEALSYNC_API_URL : null;
  const isLocalHost =
    typeof window !== "undefined" &&
  ...
  const forceLocal =
    params && (params.get("local") === "1" || params.get("api") === "local");
  const useLocalBackend =
    forceLocal || (storedApiUrl && /localhost|127\.0\.0\.1/.test(storedApiUrl));

  const activeApiBase = (
    windowApiUrl ||
    (useLocalBackend
      ? storedApiUrl && /localhost|127\.0\.0\.1/.test(storedApiUrl)
        ? storedApiUrl
        : LOCAL_API_URL
      : storedApiUrl || DEFAULT_PROD_API_URL)
  ).replace(/\/+$/, "");
