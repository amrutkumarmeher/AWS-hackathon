...
let adminSettings = {
  ...
  announcement:
    "Welcome to Hostel Mess. Please follow your counter line and keep your token ready.",
};
...
function loadLocalStore() {
  try {
    if (fs.existsSync(STORE_FILE)) {
      ...
      if (parsed.servesLog && Array.isArray(parsed.servesLog))
        servesLog = parsed.servesLog;
...
async function initDatabase() {
  ...
  if (!uri) {
    console.log(
      "No MONGODB_URI or MONGODB_URI found. Running in local file-backed mode.",
    );
  ...
  try {
    ...
    const recentServes = await servesCollection
      .find({})
      .sort({ timestamp: -1 })
      .limit(1000)
      .toArray();
    ...
    const dbSettings = await settingsCollection.findOne({
      id: "global_settings",
    });
    if (dbSettings) {
      ...
      await settingsCollection.updateOne(
        { id: "global_settings" },
        { $set: { id: "global_settings", ...adminSettings } },
        { upsert: true },
      );
    }
    ...
    console.warn(
      "MongoDB connection unavailable. Using resilient local file storage:",
      err.message,
    );
    dbState.connected = false;
...
app.get("/", (req, res) => {
  res.json({
    ...
    documentation:
      "See README.md for complete API documentation and deployment guides",
  });
});
...
app.post("/api/auth/student-login", (req, res) => {
  ...
  if (!/^\d{10}$/.test(regNo))
    return res
      ...
      .json({
        success: false,
        message: "Student Registration Number must be exactly 10 digits.",
      });
  res.json({
    success: true,
    student: { studentName: name, studentId: regNo, role: "student" },
  });
});

app.post("/api/auth/staff-login", (req, res) => {
  const name = String(req.body.staffName || "").trim();
  const staffRollNo = String(
    req.body.staffRollNo || req.body.staffId || "",
  ).trim();
  if (!name)
    return res
      .status(400)
      .json({ success: false, message: "Staff name is required." });
  if (!/^\d{5}$/.test(staffRollNo))
    return res
      .status(400)
      .json({
        success: false,
        message: "Staff Roll Number must be exactly 5 digits.",
      });
  res.json({
    success: true,
    staff: { staffName: name, staffRollNo, role: "staff" },
  });
});
...
app.post("/api/auth/admin-login", (req, res) => {
  ...
  if (!/^\d{5}$/.test(adminCode))
    return res
      .status(400)
      .json({
        success: false,
        message: "Admin security code must be exactly 5 digits.",
      });
  res.json({
    success: true,
    admin: { adminName: name, adminCode, role: "admin" },
  });
});
...
app.post("/api/admin/schedules", async (req, res) => {
  ...
  if (!mealName || !startTime || !endTime) {
    return res
      ...
      .json({
        success: false,
        message: "Meal name, start time, and end time are required.",
      });
  }
  ...
  const newSchedule = {
    ...
    counterIds:
      Array.isArray(counterIds) && counterIds.length > 0 ? counterIds : ["all"],
    enabled: true,
  };
...
app.post("/api/admin/schedules/:id/toggle", async (req, res) => {
  ...
  if (!sched)
    return res
      .status(404)
      .json({ success: false, message: "Schedule slot not found." });
...
app.delete("/api/admin/schedules/:id", async (req, res) => {
  ...
  if (schedulesCollection) {
    try {
      await schedulesCollection.deleteOne({ id: req.params.id });
    } catch (e) {}
  }
  broadcast("schedules_updated", { deletedId: req.params.id });
  res.json({
    success: true,
    message: `Removed "${removed.mealName}" schedule.`,
  });
});
    ...
    .map((c) => {
      const queue = (c.queue || []).map((s, i) => ({
        ...
        elapsedSeconds: Math.max(
          0,
          Math.floor((Date.now() - s.joinedAt) / 1000),
        ),
      }));
...
app.post("/api/counters", async (req, res) => {
  const { name, foodType, menu, staffName, staffRollNo, maxQueueSize } =
    req.body;
  if (!name || !foodType)
    return res
      .status(400)
      .json({
        success: false,
        message: "Counter name and food type are required.",
      });

  const roll =
    staffRollNo && /^\d{5}$/.test(String(staffRollNo).trim())
      ? String(staffRollNo).trim()
      : "50101";
  const items = Array.isArray(menu)
    ...
    : String(menu || "")
        ...
        .filter(Boolean);
  const maxQ =
    parseInt(maxQueueSize, 10) > 0
      ? parseInt(maxQueueSize, 10)
      : adminSettings.defaultMaxQueueSize || 20;
...
app.post("/api/counters/:id/join", async (req, res) => {
  ...
  if (!name || !regNo)
    return res
      ...
      .json({
        success: false,
        message: "Name and 10-digit registration number required.",
      });
  if (!/^\d{10}$/.test(regNo))
    return res
      .status(400)
  ...
  const target = counters.find(
    (c) => c.id === req.params.id && c.status === "active",
  );
  if (!target)
    return res
      .status(404)
      .json({
        success: false,
        message: "Mess counter is not available or has been closed.",
      });

  if (target.queue.some((s) => s.studentId === regNo)) {
    const existingPos =
      target.queue.findIndex((s) => s.studentId === regNo) + 1;
    return res.json({
      success: true,
      message: `You are already in this queue (Position #${existingPos}).`,
      counterId: target.id,
  ...
  const maxCapacity =
    target.maxQueueSize || adminSettings.defaultMaxQueueSize || 20;
  ...
  res.json({
    ...
    message: switchedFrom
      ? `Switched from "${switchedFrom}" to "${target.name}".`
      : `Joined "${target.name}".`,
    counterId: target.id,
...
app.post("/api/counters/:id/serve-next", async (req, res) => {
  ...
  if (!counter)
    return res
      .status(404)
      .json({ success: false, message: "Counter not found or inactive." });
  ...
  const duration = counter.lastServeTime
    ? Math.max(
        15,
        Math.min(120, Math.round((now - counter.lastServeTime) / 1000)),
      )
  ...
  counter.avgServeSeconds = Math.max(
    15,
    Math.round(total / counter.recentServeDurations.length),
  );
    ...
    if (idx !== -1) {
      ...
      return res.json({
        ...
        counter: {
          ...
          maxQueueSize:
            c.maxQueueSize || adminSettings.defaultMaxQueueSize || 20,
        },
        student: {
...
app.get("/api/admin/overview", (req, res) => {
  ...
  const todayCount = servesLog.filter(
    (s) => s.timestamp >= startOfDay.getTime(),
  ).length;
...
app.post("/api/admin/counters/:id/settings", async (req, res) => {
  ...
  if (staffRollNo && /^\d{5}$/.test(staffRollNo))
    counter.staffRollNo = staffRollNo.trim();
  if (menu) {
    counter.menu = Array.isArray(menu)
      ? menu
      : String(menu)
          .split(",")
          .map((s) => s.trim())
...
app.post("/api/admin/counters/:id/reopen", async (req, res) => {
  ...
  res.json({
    success: true,
    message: `Counter "${counter.name}" reopened successfully.`,
  });
});
...
app.delete("/api/admin/counters/:id", async (req, res) => {
  ...
  if (countersCollection) {
    try {
      await countersCollection.deleteOne({ id: req.params.id });
    } catch (e) {}
  }
...
app.post("/api/admin/counters/:id/kick", async (req, res) => {
  ...
  if (idx === -1)
    return res
      .status(404)
      .json({ success: false, message: "Student not in this queue." });
  ...
  res.json({
    success: true,
    message: `Removed ${removed.studentName} (${regNo}) from ${counter.name}.`,
  });
});
...
app.post("/api/admin/counters/:id/clear", async (req, res) => {
  ...
  res.json({
    success: true,
    message: `Cleared ${count} student(s) from ${counter.name}.`,
  });
});
...
app.get("/api/admin/export-serves.csv", (req, res) => {
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="mealsync_serve_logs_${Date.now()}.csv"`,
  );
  res.send(csvContent);
});
...
app.listen(PORT, HOST, async () => {
  ...
  console.log(
    `CORS enabled for origins: ${Array.isArray(allowedOrigins) ? allowedOrigins.join(", ") : allowedOrigins}`,
  );
  await initDatabase();
});
